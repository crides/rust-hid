...
impl<'a> Data<'a> {
    ...
    pub fn read<T: AsMut<[u8]>>(
        ...
        let result = if timeout.as_secs() == 0 && timeout.subsec_nanos() == 0 {
            unsafe { hid_read(self.handle.as_mut_ptr(), data.as_mut_ptr(), data.len()) }
        } else {
            unsafe {
                ...
                hid_read_timeout(
                    ...
                    data.as_mut_ptr(),
                    data.len(),
                    timeout.as_secs() as c_int * 1_000
                        + timeout.subsec_nanos() as c_int / 1_000_000,
                )
            }
        };
    ...
    pub fn read_from<T: AsMut<[u8]>>(
        &mut self,
        mut data: T,
        timeout: Duration,
    ) -> error::Result<Option<(u8, usize)>> {
        let data = data.as_mut();
